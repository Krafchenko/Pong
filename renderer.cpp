	...
	for (int y = 0; y < render_state.height; y++) {
		for (int x = 0; x < render_state.width; x++) {
			*pixel++ = 0xff00ff * x + 0x00ff00 * y;
			// you can change colors and make gradient colors. fun for showing shape functions on graphs
		}
	}
}
*/
...
// Non-retarded way to draw borders
internal void
...
draw_rect(float x, float y, float half_size_x, float half_size_y, u32 color) {
	...
	/* Scaling - the height can be changed to width ### NEEDS TO BE FIXED LATER AND BE BASED ON THE ASPECT RATIO ###
	ex. if it's wider than 16:9 height is gonna be the basis axis but if it's taller use the width */
...
// Letters
/*
* AN: needs to change the spacing (x,y) between letters (I needs more space vertically rather than horizontally)
* also small characters needs to be defined too.
*/
...
draw_text(const char* text, float x, float y, float size, u32 color) {
	...
	while (*text) {
		if (*text != 32) // Space is hardcoded
		{
			const char** letter;
			if (*text == 47) letter = letters[27];
			else if (*text == 46) letter = letters[26];
			else letter = letters[*text - 65]; // ANSI encoded ( 65 == 'A' )
			float original_x = x;
...
draw_number(int number, float x, float y, float size, u32 color) {
	...
	while (number || !drew_number) // for two digit numbers
	{
		drew_number = true;
		...
		case 0: {
			...
			draw_rect(x, y - size * 2.f, half_size, half_size, color);
			x -= size * 4.f; // moving the X position everytime we draw a digit
		} break;
